...
#     ds = ds.median(dim="time")
    # revised to skip contamination from NaNs
...
def apply_function_over_custom_times(ds, func, func_name, time_ranges):
    ...
    for timelabel, timeslice in time_ranges.items():
        ...
        if "time" in list(ds_modified.coords):
            ds_modified = ds_modified.reset_coords(
...
def feature_layers(query):
    ...
    if any(
        ...
        print(
            "Query has to include at least one of annual_geomad_times, semiannual_geomad_times, monthly_ndvi_time_range or ls_fc_cover_times"
        )
        sys.exit(1)
    ...
    annual_geomad_times = baseline_query.pop("annual_geomad_times", None)
    semiannual_geomad_times = baseline_query.pop(
        "semiannual_geomad_times", None)
    monthly_ndvi_time_range = baseline_query.pop(
        "monthly_ndvi_time_range", None)
    fc_time_ranges = baseline_query.pop("ls_fc_cover_times", None)
    ...
    # Update query to use annual_geomad_times
    if annual_geomad_times is not None:
        ds_annual_geomad_query = baseline_query.copy()
        annual_query_times = list(annual_geomad_times.values())
        annual_start_date = annual_query_times[0]
        annual_end_date = annual_query_times[-1]
        ds_annual_geomad_query.update(
            {"time": (annual_start_date, annual_end_date)})

        # load s2 annual geomad
        ...
        s2_annual_list = apply_function_over_custom_times(
            ds_s2_geomad, indices_wrapper, "s2", annual_geomad_times
        )

    # ----------------- S2 SEMIANNUAL GEOMAD -----------------

    # Update query to use semiannual_geomad_times
    if semiannual_geomad_times is not None:
        ds_semiannual_geomad_query = baseline_query.copy()
        semiannual_query_times = list(semiannual_geomad_times.values())
        semiannual_start_date = semiannual_query_times[0]
        semiannual_end_date = semiannual_query_times[-1]
        ds_semiannual_geomad_query.update(
            {"time": (semiannual_start_date, semiannual_end_date)})

        # load s2 semiannual geomad
        ...
        s2_semiannual_list = apply_function_over_custom_times(
            ds_s2_semiannual_geomad, indices_wrapper, "s2", semiannual_geomad_times
        )

    # ---------Landsat monthly NDVI anomaly--------------
    if monthly_ndvi_time_range is not None:
        monthly_ndvi_query = baseline_query.copy()
        monthly_ndvi_query.update(
            {"resampling": "bilinear", "measurements": monthly_ndvi_measurments})
        ...
        for j in range(len(list_measurements)):
            for k in range(n_time):
                ...
                measure_single = ds_monthly_ndvi[list_measurements[j]].isel(
                    time=k).rename(variable_name)
                measure_single = measure_single.reset_coords(
                ).drop_vars(["time", "spatial_ref"])
    ...
    if fc_time_ranges is not None:
        fc_query = baseline_query.copy()
        fc_query.update({"resampling": "bilinear",
                        "measurements": fc_measurements})

        fc_query_times = list(fc_time_ranges.values())
        ...
        # revised: only do masking without removing scenes
        ds_fc_clear = ds_fc.where(clear_and_dry)
    ...
    #     )
        ...
        fc_median_list = apply_function_over_custom_times(
            ds_fc_clear, mean_wrapper, "mean", fc_time_ranges
        )
